...
if($_SERVER["REQUEST_METHOD"]=='POST')
{
    ...
    $vechi_n=$_POST["p5"];
    $time=$_POST["esince1"];
    $sql="INSERT INTO `chk_in` (`Reg_number`, `User_name`, `Phone_number`, `Parking_zoon`, `vechile_name`, `Time`) VALUES ('$reg_n', '$usr', '$pho_num', '$park_z', '$vechi_n', '$time')";
        ...
        <div class="ddd" style="padding:10px;">
            <div>
             <label>Registraton Number</label><br>
                <select id="cat" name="cats" onchange="fetchemp1()">
                      <option disabled selected>Select Registraton Number</option>
                       <?php
                           ...
                           $result=mysqli_query($conn,$data);
                             while($arr=mysqli_fetch_assoc($result))
                             {
                                if($arr["parking_zone"]==$pa_area)
                                {
                                    ...
                                    $chk=$arr["User_name"];
                                    $data11="select * from `chk_in` where User_name='$chk'";
                                    $result11=mysqli_query($conn,$data11);
                        ...
                        <option value="<?php echo $arr["reg_num"];?>"> <?php echo $arr["reg_num"];?></option>
            ...
            ?>
                ...
                <label>Time</label><br>
                <input type="datetime-local" id="esince" value="<?= $date ?>"  name="esince1" required>
            </div>
            <div>
